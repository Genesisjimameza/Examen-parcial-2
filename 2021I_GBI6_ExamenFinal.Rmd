...
```{tex}
De la tabla obtenida en el apartado 1.5, se observa que los genes de mayor expresion son GATA3, PITEN, XBP1, o más bien, los 7 primeros genes tienen n=590 observaciones aociados a BCRA, es decir, exiten muchas observaciones vinculadas al carcinoma invasivo de senos. 
...
lista_genes_unicos <- combn(gen_unico,  
                            m=2,      
                            simplify = FALSE)
...
ggsave("resultados/regresion11.pdf",
       dpi = 300,
       limitsize = TRUE,)
```
...
En el boxplot3 los datos tienes una tendencia central de -1.2, por otro lado en la gráfica de regresión muestra que en los datos no hay una tendencia de la nobe de untos que se aproxime tencencia que traza la línea de la gráfiica.
```
